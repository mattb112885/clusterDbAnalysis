...
for file in $(ls | grep -v "README"); do
    echo ${file}
    cat ${file} | python ../src/raw2faa.py > ../faa/${file}.faa;
    cat ${file} | python ../src/raw2fna.py > ../fna/${file}.fna;
    cat ${file} | python ../src/raw2processed.py ../organisms > ../modtable/${file}.tsv;
done
cd ..;
...
./src/Blast_all_v_all.py faa/ blastres/ ${NCORES};
...
if [ -f aliases/aliases ]; then
    cat raw/* | grep -v "feature_id" | ./src/addAliasesToGeneNames.py aliases/aliases > db/raw_cat;
else
...
cat db/raw_cat | ./src/getNeighbors_bothStrands_rast.py  > db/neighborhoods
