...
validmethods = ['minbit', 'maxbit', 'normhsp']

usage="%prog -m [method] -c [cutoff] [options] < blast_result_table"
description = """Given a blast score table (augmented with self-bit scores for query and target genes),
calculates a similarity value based on the desired scoring metric.
Returns a three column tab-delimited table: query, target, score"""
parser = optparse.OptionParser(usage=usage, description=description)
parser.add_option("-m", "--method", help="Homology metric to use for calculation (required). Valid metrics: %s" %(" ".join(validmethods)), 
                  action="store", type="str", dest="method", default=None)
parser.add_option("-c", "--cutoff", help="Score cutoff to use (required)", action="store", type="float", dest="cutoff", default=None)
parser.add_option("-n", "--noprint", help="Set this flag if you do not want to print edges less than the cutoff as zeros (default = False - print those edges", 
                  action="store_true", dest="noprint", default=False)
